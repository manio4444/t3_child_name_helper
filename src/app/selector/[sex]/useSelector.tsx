...
export default function useSelector(
  ...
  const findSourceConfig = (sourceItem: ISelectorSourceElement) =>
    sourceItem.value === source;
  const getCurrentSourceConfig = () =>
    config[selectedSex].sources.find(findSourceConfig);
  ...
  const onDecisionsChange = (
    ...
    const decisionsUpdated = existsDecisions
      ? decisions.map((item) => {
  ...
  const triggerfetchApiData = (
    url: string | URL | Request,
    setApiData: Dispatch<SetStateAction<IApiDataItem[]>>,
    setLoadingApiData: Dispatch<SetStateAction<boolean>>,
  ) => {
  ...
  useEffect(() => {
    ...
    triggerfetchApiData(
      currentSourceValue,
      (x) => setApiData(x),
      (x) => setLoadingApiData(x),
    );
    currentSourceIndex !== undefined &&
      setDecisionsFromLocalStorage(currentSourceIndex);
